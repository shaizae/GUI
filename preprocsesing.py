...
class PreProcess:
    ...
    def grid_search(self, changes):
        ...
        for train_idx, test_idx in gss.split(self.features, self.target, self.group):  # this loop splitting dav out
            ...
            target: np.array = self.target[test_idx].reshape([-1, 1])
            group = self.group[test_idx].reshape([-1, 1])
    ...
    def uploader_feature(self, fetchers: np.array):
        """
        take a np.array and set his indexes
        :param fetchers: the array of indexes (type :np.array)
        :return: None
        """
        self.features = self.features[:, fetchers]
        self.features_name = self.features_name[fetchers]
        self.features_size = fetchers.shape[1]
